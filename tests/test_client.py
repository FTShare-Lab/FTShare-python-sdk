...
from ftshare import FtshareAPIError, FtshareDecodeError, FtshareDownloadError, FtshareHTTPError
...
ARCHIVE_YEAR = 2023
ARCHIVE_BODY = b"AAABBB"
ARCHIVE_SHA = hashlib.sha256(ARCHIVE_BODY).hexdigest()
ARCHIVE_ETAG = '"etag-1"'
ARCHIVE_LIST_URL = "https://market.ft.tech/gateway/api/v2/market/data/kline-archives"
ARCHIVE_DOWNLOAD_URL = ARCHIVE_LIST_URL + "/2023/download"
ARCHIVE_FILENAME = "ftshare_1m_2023.tar.zst"


def archive_listing(size_bytes=len(ARCHIVE_BODY), sha256=ARCHIVE_SHA, year=ARCHIVE_YEAR):
    return {
        "code": 200,
        "message": "success",
        "data": [
            {
                ...
                "last_modified": "2026-09-18T12:25:24.434+00:00",
                "sha256": sha256,
            }
        ],
    }
...
def archive_response(status_code=200, body=ARCHIVE_BODY, etag=ARCHIVE_ETAG, headers=None, chunks=None):
    response_headers = {"ETag": etag}
    response_headers.update(headers or {})
    return FakeResponse(status_code=status_code, payload={}, content=body, headers=response_headers, chunks=chunks)


def archive_part(tmp_path, body=b"AAA", etag=ARCHIVE_ETAG, url=ARCHIVE_DOWNLOAD_URL, meta=True):
    part = tmp_path / f"{ARCHIVE_FILENAME}.part"
    part.write_bytes(body)
    if meta:
        (tmp_path / f"{ARCHIVE_FILENAME}.part.meta").write_text(
            json.dumps({"url": url, "etag": etag, "size": len(body)}), encoding="utf-8"
        )
    return part


def test_kline_archives_returns_listing_rows():
    session = FakeSession([FakeResponse(payload=archive_listing())])
    client = FtshareClient(session=session)

    rows = client.kline_archives(as_dataframe=False)

    assert rows[0]["year"] == ARCHIVE_YEAR
    assert rows[0]["sha256"] == ARCHIVE_SHA
    assert session.calls[0]["url"] == ARCHIVE_LIST_URL
    assert session.calls[0]["params"] == {}


def test_kline_archives_download_rejects_year_absent_from_listing(tmp_path):
    session = FakeSession([FakeResponse(payload=archive_listing())])
    client = FtshareClient(session=session)

    with pytest.raises(FtshareDownloadError, match="no archive package is offered for year 2019"):
        client.kline_archives_download(2019, save_dir=tmp_path)

    assert len(session.calls) == 1
    assert list(tmp_path.iterdir()) == []


def test_kline_archives_download_rejects_non_four_digit_year(tmp_path):
    session = FakeSession([])
    client = FtshareClient(session=session)

    for bad_year in ("23", "abcd", 20233):
        with pytest.raises(ValueError, match="4-digit year"):
            client.kline_archives_download(bad_year, save_dir=tmp_path)

    assert session.calls == []


def test_kline_archives_download_streams_and_verifies(tmp_path):
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing()),
            archive_response(chunks=[b"AAA", b"BBB"]),
        ]
    )
    client = FtshareClient(session=session, api_key="test-key")

    saved = client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path)

    target = tmp_path / ARCHIVE_FILENAME
    assert saved == str(target)
    assert target.read_bytes() == ARCHIVE_BODY
    assert list(tmp_path.iterdir()) == [target]

    download_call = session.calls[1]
    assert download_call["url"] == ARCHIVE_DOWNLOAD_URL
    assert download_call["stream"] is True
    assert download_call["headers"] == {"FTSHARE_API_KEY": "test-key"}
    assert "json" not in download_call


def test_kline_archives_download_resumes_from_partial_file(tmp_path):
    archive_part(tmp_path)
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing()),
            archive_response(status_code=206, body=b"BBB", headers={"Content-Range": "bytes 3-5/6"}, chunks=[b"BBB"]),
        ]
    )
    client = FtshareClient(session=session)
    ...
    assert Path(saved).read_bytes() == ARCHIVE_BODY
    assert list(tmp_path.iterdir()) == [tmp_path / ARCHIVE_FILENAME]
    assert session.calls[1]["headers"] == {"Range": "bytes=3-", "If-Range": ARCHIVE_ETAG}


def test_kline_archives_download_discards_untraceable_partial_file(tmp_path):
    archive_part(tmp_path, url="https://market.ft.tech/gateway/api/v2/market/data/other/download")
    session = FakeSession([FakeResponse(payload=archive_listing()), archive_response(chunks=[ARCHIVE_BODY])])
    client = FtshareClient(session=session)

    saved = client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path)

    assert Path(saved).read_bytes() == ARCHIVE_BODY
    assert "Range" not in (session.calls[1]["headers"] or {})


def test_kline_archives_download_restarts_when_server_etag_changed(tmp_path):
    archive_part(tmp_path, etag='"stale"')
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing(size_bytes=6, sha256=hashlib.sha256(b"CCCCCC").hexdigest())),
            archive_response(body=b"CCCCCC", etag='"fresh"', chunks=[b"CCCCCC"]),
        ]
    )
    client = FtshareClient(session=session)
    ...
    # A 200 answer to a stale If-Range must truncate, not append.
    assert Path(saved).read_bytes() == b"CCCCCC"
    assert session.calls[1]["headers"]["If-Range"] == '"stale"'


def test_kline_archives_download_restarts_after_unsatisfiable_range(tmp_path):
    archive_part(tmp_path)
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing()),
            FakeResponse(status_code=416, payload={}, headers={"Content-Range": "bytes */6"}),
            archive_response(chunks=[ARCHIVE_BODY]),
        ]
    )
    client = FtshareClient(session=session)
    ...
    assert "Range" not in (session.calls[2]["headers"] or {})


def test_kline_archives_download_retries_after_connection_error(tmp_path):
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing()),
    ...
    saved = client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path, retries=1)

    assert Path(saved).read_bytes() == ARCHIVE_BODY
    assert len(session.calls) == 3


def test_kline_archives_download_raises_when_retries_exhausted(tmp_path):
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing()),
    ...
    with pytest.raises(requests.ConnectionError):
        client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path, retries=1)

    assert len(session.calls) == 3
    assert not (tmp_path / ARCHIVE_FILENAME).exists()


def test_kline_archives_download_rejects_size_mismatch(tmp_path):
    session = FakeSession([FakeResponse(payload=archive_listing(size_bytes=99)), archive_response(chunks=[ARCHIVE_BODY])])
    client = FtshareClient(session=session)

    with pytest.raises(FtshareDownloadError, match="size mismatch"):
        client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path)

    assert list(tmp_path.iterdir()) == []


def test_kline_archives_download_rejects_sha256_mismatch(tmp_path):
    session = FakeSession([FakeResponse(payload=archive_listing(sha256="0" * 64)), archive_response(chunks=[ARCHIVE_BODY])])
    client = FtshareClient(session=session)

    with pytest.raises(FtshareDownloadError, match="sha256 mismatch"):
        client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path)

    # The corrupt partial file is dropped so a later call cannot resume from it.
    assert list(tmp_path.iterdir()) == []


def test_kline_archives_download_skips_existing_verified_file(tmp_path):
    target = tmp_path / ARCHIVE_FILENAME
...
def test_kline_archives_download_does_not_retry_not_found(tmp_path):
    session = FakeSession(
        [
            FakeResponse(payload=archive_listing()),
            FakeResponse(status_code=404, payload={}, text='{"code":404,"message":"not found"}'),
        ]
    )
    client = FtshareClient(session=session)

    with pytest.raises(FtshareHTTPError):
        client.kline_archives_download(ARCHIVE_YEAR, save_dir=tmp_path, retries=3)
